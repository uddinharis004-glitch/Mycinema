        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", fontSize:11, color:"#6B7280" }}>
          ...
          {confirmDel
            ? <div style={{ display:"flex", gap:5 }}>
                <button onClick={handleDelete} disabled={deleting} style={{ background:"rgba(239,68,68,.15)", border:"1px solid rgba(239,68,68,.4)", color:"#EF4444", fontSize:11, padding:"2px 8px", borderRadius:4, cursor:"pointer", fontFamily:"inherit" }}>
                  {deleting ? "…" : "Delete"}
                </button>
                <button onClick={(e) => { e.stopPropagation(); setConfirmDel(false); }} style={{ background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", color:"#9CA3AF", fontSize:11, padding:"2px 8px", borderRadius:4, cursor:"pointer", fontFamily:"inherit" }}>
                  Cancel
                </button>
              </div>
            : <button onClick={(e) => { e.stopPropagation(); setConfirmDel(true); }} style={{ background:"transparent", border:"none", color:"#6B7280", cursor:"pointer", fontSize:14, padding:"1px 4px", fontFamily:"inherit" }}>
                🗑
              </button>
          }
        </div>
...
function UploadPanel({ onDone }) {
  ...
  const [phase, setPhase] = useState(""); // "video" | "thumb" | "done"
  ...
  const handleFile = useCallback(async (f) => {
    if (!/\.(mp4|mkv|avi|mov|webm|wmv|m4v|flv)$/i.test(f.name) && !f.type.startsWith("video/")) {
      setError("Please select a video file (MP4, MKV, MOV, AVI, WebM…)");
      return;
    }
    setError(null);
    setFile(f);
  ...
  const upload = async () => {
    ...
    try {
      ...
      // 1. Upload video
      setPhase("video");
      const { url: videoSignedUrl } = await api.getUploadUrl(filename, file.type || "video/mp4");
      await uploadToR2(videoSignedUrl, file, setProgress);

      // 2. Upload thumbnail
      if (thumbBlob) {
        setPhase("thumb");
        setProgress(0);
        const { url: thumbSignedUrl } = await api.getUploadUrl(
          filename.replace(/\.[^.]+$/, ".jpg"), "image/jpeg"
        );
        await uploadToR2(thumbSignedUrl, thumbBlob, setProgress);
      }
    ...
    <div style={{ maxWidth:660, margin:"0 auto" }}>
      {!file ? (
        ...
        >
          ...
          <div style={{ fontSize:22, fontWeight:800, color:"#F5F5F5", marginBottom:10 }}>
            {dragging ? "Release to upload" : "Upload a Video"}
          </div>
          <div style={{ fontSize:14, color:"#6B7280", lineHeight:1.8, marginBottom:28 }}>
            Drag & drop or click to browse<br/>
            MP4, MKV, MOV, AVI, WebM · Uploads directly to <strong style={{ color:"#9CA3AF" }}>Cloudflare R2</strong>
          </div>
          <button
        ...
        <div style={{ background:"#12141B", borderRadius:16, padding:28, border:"1px solid rgba(255,255,255,.08)" }}>
          ...
          {uploading && (
            <div style={{ marginBottom:20 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#9CA3AF", marginBottom:6 }}>
                <span>{phaseLabel[phase] || "Preparing…"}</span>
                <span>{progress}%</span>
              </div>
          ...
          <div style={{ display:"flex", gap:12 }}>
            <button
              ...
              style={{ flex:1, background: phase === "done" ? "rgba(16,185,129,.9)" : "linear-gradient(135deg,#F59E0B,#EF4444)", border:"none", color:"#000", fontWeight:700, fontSize:14, padding:11, borderRadius:9, cursor: uploading || !title.trim() ? "not-allowed" : "pointer", opacity: uploading || !title.trim() ? .7 : 1, transition:"background .3s", fontFamily:"inherit" }}
            >
              {uploading ? (phase === "done" ? "✅ Done!" : "Uploading…") : "⬆ Upload to R2"}
            </button>
            {!uploading && (
              <button onClick={reset} style={{ background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.12)", color:"#9CA3AF", fontSize:14, padding:"11px 20px", borderRadius:9, cursor:"pointer", fontFamily:"inherit" }}>
                Cancel
              </button>
            )}
          </div>
...
export default function CineVault() {
  ...
  const handleUploadDone = () => {
    setView("library");
    loadVideos(); // refresh immediately
  };
    ...
    <div style={{ minHeight:"100vh", background:"#09090E", color:"#E8EAF2", fontFamily:"Inter,system-ui,sans-serif" }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{width:6px;background:#13141C}
          ...
          <div style={{ display:"flex", gap:3, background:"rgba(255,255,255,.06)", borderRadius:9, padding:3 }}>
            {[["library","📽 Library"],["upload","⬆ Upload"]].map(([key, label]) => (
              <button key={key} onClick={() => setView(key)}
                style={{ padding:"5px 20px", borderRadius:7, border:"none", cursor:"pointer", fontSize:13, fontWeight:600, background: view===key ? "rgba(245,158,11,.88)" : "transparent", color: view===key ? "#000" : "#9CA3AF", transition:"all .15s" }}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            {view === "library" && (
              <button onClick={loadVideos} title="Refresh"
                style={{ background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.1)", color:"#9CA3AF", width:32, height:32, borderRadius:8, cursor:"pointer", fontSize:14, display:"flex", alignItems:"center", justifyContent:"center" }}>
                ↻
              </button>
            )}
          </div>
          ...
          <div className="fade">
            ...
            {!loading && loadError && (
              <div style={{ textAlign:"center", padding:"80px 0" }}>
                ...
                <div style={{ fontSize:13, color:"#EF4444", marginBottom:8, maxWidth:480, margin:"0 auto 20px" }}>{loadError}</div>
                <div style={{ fontSize:12, color:"#6B7280", marginBottom:20 }}>Make sure your environment variables are set in Vercel.</div>
                <button onClick={loadVideos} style={{ background:"rgba(245,158,11,.9)", border:"none", color:"#000", fontWeight:700, padding:"8px 22px", borderRadius:8, cursor:"pointer", fontFamily:"inherit" }}>
                  Retry
                </button>
              </div>
            )}
            ...
            {!loading && !loadError && filtered.length > 0 && (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:20 }}>
                {filtered.map((v) => (
                  <VideoCard key={v.id} video={v} onPlay={setPlaying} onDelete={handleDelete} />
                ))}
              </div>
            )}
            ...
            {!loading && !loadError && search && filtered.length === 0 && (
              <div style={{ textAlign:"center", padding:"60px 0", color:"#6B7280" }}>
                ...
                <div style={{ fontWeight:600, color:"#E8EAF2", marginBottom:6 }}>No results for "{search}"</div>
              </div>
            )}
          </div>
        )}
